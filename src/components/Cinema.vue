  ...
  <div>
    <div class="cinema">
      <preview
        ...
        :selected-seats-prop="selectedSeats"
        @start-reservation="startReservation"
      />
      <div class="screen"></div>
      <div class="rows">
        <row
          v-for="(row, index) in cinema.left"
          :key="`row-left-${index}`"
          :row-prop="row"
          side-prop="left"
          @seat-selected="collectSeats($event)"
        />
      </div>
      <div class="rows">
        <row
          v-for="(row, index) in cinema.right"
          :key="`row-right-${index}`"
          :row-prop="row"
          side-prop="right"
          @seat-selected="collectSeats($event)"
...
<script>
  ...
  export default {
    ...
    methods: {
      collectSeats ($event) {
        const { id, type } = $event;
        this.selectedSeats = (type === 'add') ? [...this.selectedSeats, id] : this.selectedSeats.filter(_id => _id !== id)
      },
